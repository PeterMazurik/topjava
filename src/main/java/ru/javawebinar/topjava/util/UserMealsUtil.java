...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        HashMap<LocalDate, Integer> allMealsCounter = new HashMap<>();

        for (UserMeal meal : meals) {
            LocalDate key = meal.getDateTime().toLocalDate();
            allMealsCounter.merge(key, meal.getCalories(), Integer::sum);
        }

        for (UserMeal meal : meals) {
            LocalDate key = meal.getDateTime().toLocalDate();
            boolean isTime = TimeUtil.isBetweenHalfOpen(meal.getDateTime().toLocalTime(), startTime, endTime);
            if (isTime) {
                if (allMealsCounter.get(key) > caloriesPerDay) {
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> allMealsCounter = meals.stream()
                .collect(Collectors.toMap(x -> x.getDateTime().toLocalDate(), UserMeal::getCalories, Integer::sum));

        return meals.stream()
                .filter(meal -> TimeUtil.isBetweenHalfOpen(meal.getDateTime().toLocalTime(), startTime, endTime))
                .map(meal -> {
                    if (allMealsCounter.get(meal.getDateTime().toLocalDate()) > caloriesPerDay) {
